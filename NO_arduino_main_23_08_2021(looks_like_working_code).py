...
IN4 = timer.channel(4, Timer.PWM, pin=Pin('P5'))
...
straight_angle = 1800 # ??????????????????????????????????????????????????????????????????????? 1550
right_angle = straight_angle - 450 # ?????????????????????????????????????????????????????????? 1100
left_angle = straight_angle + 450 # ??????????????????????????????????????????????????????????? 2000
cube_bypassing = f
negative_value = f
button_state = f
STOP = t
...
while(True):
    ...
    img = img.gaussian(3)
    img = img.gamma_corr(gamma = 0.5, contrast = 18.0, brightness = -0.4)
    nearest_cube = 0
    nearest_cube_area = 0
    cube_error = 0
